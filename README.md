...
- **로그인 필요.** 이메일+비밀번호로 **회원가입/로그인**한 사용자만 접근한다(별도 승인 절차 없음). 비밀번호 분실 시 이메일로 재설정.
- 인증은 **Supabase Auth**, 접근 제어는 **RLS**(로그인 사용자만)가 서버측에서 강제한다.
- 데이터는 **Supabase**(Postgres) 백엔드에 저장되어 로그인 사용자들이 공유한다.
